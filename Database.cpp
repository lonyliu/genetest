...
int Read_data(PolyModQ**& DataPoly0, PolyModQ**& DataPoly1, PolyModQ**& DataPoly2,char* filename){
  ...
  // for (i = 0; i < nLine; i++) cout << strChr[i] << '\t' << strPOS[i] << '\t' << strREF[i] << '\t' << strALT[i] << endl;
    ...
    bool ** check2 = new bool*[cnt];//0000000000005
    for(i = 0; i < cnt; ++i){
        check0[i]= new bool[N];
        check1[i]= new bool[N];
        check2[i]= new bool[N];
    ...
    ZmodQ* deg2 = new ZmodQ[nLine];

    // i=0
    Encode_deg(deg0[0], deg1[0],deg2[0], strChr[0], strPOS[0]);
    check0[0][deg0[0]] = 1;
    check1[0][deg1[0]] = 1;
    check2[0][deg2[0]] = 1;
    ...
    for(j = 0; j < nSNP; ++j){
        ...
        DataPoly1[0][j][deg1[0]] = unif_T(gen);
        DataPoly2[0][j][deg2[0]] = SNP[0][j] - DataPoly0[0][j][deg0[0]]-DataPoly1[0][j][deg1[0]];
    }
...
*/
    for(k = 1; k < nLine; ++k){
        Encode_deg(deg0[k], deg1[k],deg2[k], strChr[k], strPOS[k]);
        i = 0;
        while(check0[i][deg0[k]] == 1 && check1[i][deg1[k]] == 1 && check2[i][deg2[k]] == 1){ i++; }
        if (i == nPoly) { nPoly++; }

        if(check0[i][deg0[k]] == 0 && check1[i][deg1[k]] == 0 && check2[i][deg2[k]] == 0){
            for(j = 0; j < nSNP; ++j){
                DataPoly0[i][j][deg0[k]] = unif_T(gen);
                DataPoly1[i][j][deg1[k]] = unif_T(gen);
                DataPoly2[i][j][deg2[k]] = SNP[k][j] - DataPoly0[i][j][deg0[k]]-DataPoly1[i][j][deg1[k]];
            }
        }
        else if(check0[i][deg0[k]] == 0 && check1[i][deg1[k]] == 0 && check2[i][deg2[k]] == 1){
            for(j = 0; j < nSNP; ++j){
                DataPoly1[i][j][deg1[k]] = unif_T(gen);

                DataPoly0[i][j][deg0[k]] = SNP[k][j] - DataPoly1[i][j][deg1[k]]-DataPoly2[i][j][deg2[k]]; }
        }
        else if(check0[i][deg0[k]] == 0 && check1[i][deg1[k]] == 1 && check2[i][deg2[k]] == 0){
            for(j = 0; j < nSNP; ++j){
                DataPoly2[i][j][deg2[k]] = unif_T(gen);
                DataPoly0[i][j][deg0[k]] = SNP[k][j] - DataPoly1[i][j][deg1[k]]-DataPoly2[i][j][deg2[k]]; }
        }
        else if(check0[i][deg0[k]] == 1 && check1[i][deg1[k]] == 0 && check2[i][deg2[k]] == 0){
            for(j = 0; j < nSNP; ++j){
                DataPoly2[i][j][deg2[k]] = unif_T(gen);
                DataPoly1[i][j][deg1[k]] = SNP[k][j] - DataPoly0[i][j][deg0[k]]-DataPoly2[i][j][deg2[k]]; }
        }
        else if(check0[i][deg0[k]] == 0 && check1[i][deg1[k]] == 1 && check2[i][deg2[k]] == 1){
            for(j = 0; j < nSNP; ++j){

                DataPoly0[i][j][deg0[k]] = SNP[k][j] - DataPoly1[i][j][deg1[k]]-DataPoly2[i][j][deg2[k]]; }
        }
        else if(check0[i][deg0[k]] == 1 && check1[i][deg1[k]] == 0 && check2[i][deg2[k]] == 1){
            for(j = 0; j < nSNP; ++j){

                DataPoly1[i][j][deg1[k]] = SNP[k][j] - DataPoly0[i][j][deg0[k]]-DataPoly2[i][j][deg2[k]]; }
        }
        else if(check0[i][deg0[k]] == 1 && check1[i][deg1[k]] == 1 && check2[i][deg2[k]] == 0){
            for(j = 0; j < nSNP; ++j){

                DataPoly2[i][j][deg2[k]] = SNP[k][j] - DataPoly1[i][j][deg1[k]]-DataPoly0[i][j][deg0[k]]; }
        }

        check0[i][deg0[k]] = 1; check1[i][deg1[k]] = 1;check2[i][deg2[k]] = 1;
    }
